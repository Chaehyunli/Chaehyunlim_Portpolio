    ...
    decisions: [
      {
        ...
        considerations: [
          ...
          "서버 마스터키로 암호화(Fernet) — 구현은 단순하지만 서버 침해 시 저장된 모든 키를 복호화할 수 있음",
        ],
        solution: [
          {
      ...
      },
      {
        ...
        solution: [
          ...
          },
          {
            ...
            detail:
              "\"해산물 태그와 이자카야 카테고리가 의미적으로 겹치는가\" 같은 판단만 LLM에 남기고, 규칙을 통과한 후보에만 1회 호출하도록 구성했다.",
          },
          {
      ...
      },
      {
        ...
        problem:
          "네이버 지역검색은 초당 10건·일 25,000건, ODsay는 일 1,000건 한도가 있다. 장소 풀을 모으려면 카테고리 16종 × 태그만큼 팬아웃해야 해서, 한 번의 일정 생성이 네이버를 36회 호출한다(경기 수원 조건 실측, 고유 장소 92곳).",
        considerations: [
        ...
        solution: [
          {
            ...
            detail:
              "초당 상한은 세션 단위 라운드로빈 토큰버킷 — 세션 N개면 세션당 처리량이 자연히 rate/N으로 수렴한다. 일일 한도는 Redis 카운터 + 자정 TTL로 여러 워커에서도 전역 집계가 하나로 유지된다.",
          },
          {
